...
CliOptions parse_cli(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg.rfind("--eval_steps", 0) == 0) {
            opts.eval_steps = std::stoi(get_val("--eval_steps"));
...
int main(int argc, char** argv) {
    try {
        ...
        if (cli.pretokenized_path.empty()) {
            tokenizer = std::make_unique<HFTokenizer>(cli.model_dir, true);
            if (!tokenizer->load()) {
        ...
        trainer_cfg.eval_steps = std::max(0, cli.eval_steps);
