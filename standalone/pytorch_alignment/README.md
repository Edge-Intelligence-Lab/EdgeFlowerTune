...
Current GPT-2 and Gemma defaults are alignment-first:
- CPU by default
- no shuffle by default
- `seq_len=128`, `batch=8`, `grad_accum=1`
- `lr=2e-4`, warmup disabled by default
- dropout disabled by default for GPT-2, `lora_dropout=0.0` by default for Gemma
...
**Mirrors:** `qwen_lora_finetune/src/main.cpp`

Supports WikiText-2 (default), JSONL masked causal LM, and fixed-batch alignment.
Current defaults are alignment-first:
- `seq_len=128`, `batch=8`, `grad_accum=1`
- `lr=2e-4`, `warmup_steps=0`
- `target_mode=qv`, `lora_dropout=0.0`
- no shuffle by default
- strict raw-text alignment should prefer `--pretokenized_path data/wikitext2/pretokenized_qwen/wt2_qwen_tokens.bin`

**Usage (WikiText-2):**
```bash
python PT_code/pytorch_alignment/qwen_lora_finetune.py \
  --model_dir /path/to/Qwen2.5-0.5B \
  --pretokenized_path data/wikitext2/pretokenized_qwen/wt2_qwen_tokens.bin \
  --pretokenized_meta data/wikitext2/pretokenized_qwen/meta.json \
  --output_dir pytorch_runs/qwen_wikitext_lora \
  --epochs 1 --batch 8 --seq_len 128 \
  --target_mode qv --lora_r 8 --lora_alpha 16 \
  --lora_dropout 0.0 --no_shuffle
```
...
python PT_code/pytorch_alignment/qwen_lora_finetune.py \
  ...
  --target_mode qv --lora_dropout 0.0 --no_shuffle
```
