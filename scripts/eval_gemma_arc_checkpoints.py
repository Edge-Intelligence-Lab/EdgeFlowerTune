...
def load_gemma_lora(model: torch.nn.Module, lora_path: Path) -> None:
    if lora_path.is_dir():
        ...
        if filtered_missing:
            raise ValueError(f"Failed to load {lora_path.name}: missing={filtered_missing[:8]} unexpected={unexpected[:8]}")
        return
    ...
    with jsonl_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                continue
            examples.append({"question": ex["question"], "choices": texts, "gold": gold})
    return examples
...
def prepare_examples(tokenizer, examples: List[Dict[str, object]], max_examples: int = 0) -> List[Dict[str, object]]:
    ...
    for ex in examples:
        ...
        candidates = [encode_pair(tokenizer, prompt, normalize_text(str(choice))) for choice in ex["choices"]]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Gemma ARC checkpoints.")
    ...
    prepared_examples = prepare_examples(tokenizer, read_examples(Path(args.arc_jsonl)), args.max_examples)

    output_jsonl = Path(args.output_jsonl)
    output_csv = Path(args.output_csv)
    rows = read_existing_rows(output_jsonl)
    ...
    for ckpt_name, step, ckpt_path in checkpoints:
        ...
        if (ckpt_name, str(step_value)) in completed:
            continue
        load_gemma_lora(model, ckpt_path)
        metrics = evaluate_arc(model, prepared_examples, device, args.batch_size)
        row = {
