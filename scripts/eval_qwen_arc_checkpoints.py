...
def load_qwen_lora(model: torch.nn.Module, lora_path: Path) -> None:
    if lora_path.is_dir():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Qwen ARC checkpoints.")
    ...
    for ckpt_name, step, ckpt_path in checkpoints:
        ...
        if (ckpt_name, str(step_value)) in completed:
            continue
        load_qwen_lora(model, ckpt_path)
