...
PT_STEP_DIR_RE = re.compile(r"(?:adapter_model|lora\.safetensors)_step(\d+)$")
...
def iter_checkpoints(
    ...
    if include_final:
        ...
        for final_dir_name in ("adapter_model", "lora.safetensors"):
            final_dir = run_dir / final_dir_name
            if (final_dir / "adapter_model.safetensors").exists():
                items.append((final_dir.name, None, final_dir))
    return items
...
def read_examples(jsonl_path: Path, labels_path: Path) -> List[Dict[str, object]]:
...
def prepare_examples(
    ...
    for ex in examples:
        goal = " ".join(str(ex["goal"]).strip().split())
        prompt = f"Goal: {goal}\nSolution:"
        candidates = [
            encode_pair(tokenizer, prompt, str(ex["sol1"])),
            encode_pair(tokenizer, prompt, str(ex["sol2"])),
        ]
...
def evaluate_piqa(
    ...
    correct_raw = 0
    correct_norm = 0
    scores_raw_by_example: List[List[float]] = [[0.0, 0.0] for _ in range(total)]
    scores_norm_by_example: List[List[float]] = [[0.0, 0.0] for _ in range(total)]
    ...
    for ex_idx, ex in enumerate(prepared_examples):
        gold = int(ex["gold"])
        pred_raw = max(range(2), key=lambda i: scores_raw_by_example[ex_idx][i])
        pred_norm = max(range(2), key=lambda i: scores_norm_by_example[ex_idx][i])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate GPT-2 PIQA checkpoints from C++ LoRA safetensors.")
    parser.add_argument("--run_dir", required=True)
    parser.add_argument("--piqa_jsonl", required=True)
    parser.add_argument("--piqa_labels", required=True)
    ...
    examples = read_examples(Path(args.piqa_jsonl), Path(args.piqa_labels))
