...
bool load_npy_int32(const std::string& path,
                    std::vector<int64_t>& shape_out,
    ...
    std::string header(header_len, '\0');
    in.read(header.data(), header_len);
    if (header.find("<i4") == std::string::npos && header.find("|i4") == std::string::npos) return false;
    auto l = header.find('(');
    auto r = header.find(')', l);
    if (l == std::string::npos || r == std::string::npos) return false;
    std::string shape_str = header.substr(l + 1, r - l - 1);
    ...
    for (auto d : shape_out) count *= static_cast<size_t>(d);
    data_out.resize(count);
    in.read(reinterpret_cast<char*>(data_out.data()), static_cast<std::streamsize>(count * sizeof(int32_t)));
    return in.good();
}
...
struct CliOptions {
    ...
    std::string init_lora_path;
    std::string export_lora_path;
...
CliOptions parse_cli(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.rfind("--model_dir", 0) == 0) {
        ...
        } else if (arg.rfind("--fixed_batch_dir", 0) == 0) {
            opts.fixed_batch_dir = get_val("--fixed_batch_dir");
        } else if (arg.rfind("--fixed_batch_steps", 0) == 0) {
            opts.fixed_batch_steps = std::stoi(get_val("--fixed_batch_steps"));
        ...
        } else if (arg == "--export_lora_only") {
            opts.export_lora_only = true;
        } else if (arg.rfind("--export_lora_only", 0) == 0) {
            auto v = get_val("--export_lora_only");
            opts.export_lora_only = (v != "0" && v != "false" && v != "False");
        } else if (arg.rfind("--init_lora", 0) == 0) {
            opts.init_lora_path = get_val("--init_lora");
        } else if (arg.rfind("--export_lora", 0) == 0) {
            opts.export_lora_path = get_val("--export_lora");
        ...
        } else if (arg.rfind("--lora_r", 0) == 0) {
            opts.lora_r = std::stoi(get_val("--lora_r"));
        } else if (arg.rfind("--lora_alpha", 0) == 0) {
            opts.lora_alpha = std::stof(get_val("--lora_alpha"));
        ...
        } else if (arg.rfind("--lr", 0) == 0 || arg.rfind("--learning_rate", 0) == 0) {
            opts.learning_rate = std::stof(get_val(arg));
        ...
        } else if (arg.rfind("--logging_steps", 0) == 0) {
            opts.logging_steps = std::stoi(get_val("--logging_steps"));
            ...
            opts.shuffle_train = true;
        } else if (arg == "--no_shuffle") {
            opts.shuffle_train = false;
        } else if (arg.rfind("--tokenizer_json", 0) == 0) {
            opts.tokenizer_json = get_val("--tokenizer_json");
...
int main(int argc, char** argv) {
    try {
        ...
        if (cli.pretokenized_path.empty()) {
            ...
            tokenizer = std::make_unique<HFTokenizer>(tok_path, use_model_dir);
            if (!tokenizer->load()) {
        ...
        lora_spec.rank = std::max(1, cli.lora_r);
        lora_spec.alpha = cli.lora_alpha;
        lora_spec.dropout = cli.lora_dropout;
        if (!cli.align_dump_dir.empty() || !cli.fixed_batch_dir.empty()) {
        ...
        if (!cli.init_lora_path.empty()) {
            std::cout << "[Init] Loading LoRA adapter from " << cli.init_lora_path << std::endl;
            injector.load_lora_safetensors(cli.init_lora_path);
        }

        const std::string final_lora_path =
            cli.export_lora_path.empty()
                ? (cli.output_dir + "/gemma_lora.safetensors")
                : cli.export_lora_path;

        if (cli.export_lora_only) {
            std::cout << "[Save] Exporting current LoRA adapter to " << final_lora_path << std::endl;
            const auto export_parent = std::filesystem::path(final_lora_path).parent_path();
            if (!export_parent.empty()) {
                std::filesystem::create_directories(export_parent);
            }
            injector.save_lora_safetensors(final_lora_path);
            std::cout << "🎉 Gemma LoRA export finished" << std::endl;
            return 0;
        }
        ...
        // Fixed-batch training (strict alignment on identical tokens)
        if (!cli.fixed_batch_dir.empty()) {
            std::vector<int64_t> ids_shape, attn_shape, labels_shape;
            std::vector<int32_t> ids_data, labels_data;
            std::vector<float> attn_data;
            std::string ids_path = cli.fixed_batch_dir + "/input_ids.npy";
            std::string attn_path = cli.fixed_batch_dir + "/attention_mask.npy";
            std::string labels_path = cli.fixed_batch_dir + "/labels.npy";

            if (!load_npy_int32(ids_path, ids_shape, ids_data)) {
                throw std::runtime_error("Failed to load input_ids.npy from " + cli.fixed_batch_dir);
            }
            if (!load_npy_float32(attn_path, attn_shape, attn_data)) {
                throw std::runtime_error("Failed to load attention_mask.npy from " + cli.fixed_batch_dir);
            }
            if (!load_npy_int32(labels_path, labels_shape, labels_data)) {
                throw std::runtime_error("Failed to load labels.npy from " + cli.fixed_batch_dir);
            }
            if (ids_shape != attn_shape || ids_shape != labels_shape) {
                throw std::runtime_error("Fixed batch shape mismatch (ids/attn/labels)");
            }
            if (ids_shape.size() != 2) {
                throw std::runtime_error("Fixed batch must be 2D [B,S]");
            }

            auto input_ids = from_blob(ids_data.data(), ids_shape, kInt32, kCPU);
            auto attention_mask = from_blob(attn_data.data(), attn_shape, kFloat32, kCPU);
            auto labels = from_blob(labels_data.data(), labels_shape, kInt32, kCPU);

            int steps = (cli.fixed_batch_steps > 0) ? cli.fixed_batch_steps : 1;
            std::cout << "[FixedBatch] dir=" << cli.fixed_batch_dir
            ...
            acfg.beta2 = 0.999f;
            acfg.epsilon = 1e-8f;
            acfg.weight_decay = cli.weight_decay;
            acfg.clip_grad_norm = cli.max_grad_norm;
            Adam opt(acfg);
            ...
            auto get_lr = [&](int step1) -> float {
                int total_updates = std::max(1, steps);
                int warmup_steps = static_cast<int>(total_updates * cli.warmup_ratio);
                if (warmup_steps > 0 && step1 <= warmup_steps) {
                    return cli.learning_rate * (static_cast<float>(step1) / warmup_steps);
                }
                float progress = static_cast<float>(step1 - warmup_steps) /
                                 std::max(1, total_updates - warmup_steps);
                progress = std::clamp(progress, 0.0f, 1.0f);
                return cli.learning_rate * (1.0f - progress);
            };
            ...
            for (int step = 1; step <= steps; ++step) {
                ...
                if (cli.max_grad_norm > 0.0f) {
                    opt.clip_grad_norm(params_vec, cli.max_grad_norm);
                }

                float lr = get_lr(step);
                opt.set_learning_rate(lr);
                grads_vec.clear();
                for (auto& p : params_vec) grads_vec.push_back(p->grad());
                opt.step(params_vec, grads_vec);
                for (auto& p : params_vec) p->zero_grad();

                std::cout << "[FixedBatch] step " << step << "/" << steps
                          << " lr " << lr << " loss " << loss_val << std::endl;
            }
            return 0;
        }
        ...
        data_cfg.shuffle_train = cli.shuffle_train;
        ...
        if (!cli.jsonl_train.empty() || !cli.jsonl_valid.empty() || !cli.jsonl_test.empty()) {
            ...
            int eos_id = tokenizer ? tokenizer->token_to_id("<eos>") : -1;
            if (eos_id < 0 && tokenizer) eos_id = tokenizer->token_to_id("</s>");
            int pad_id = tokenizer ? tokenizer->token_to_id("<pad>") : -1;
            data_cfg.eos_id = eos_id;
        ...
        trainer_cfg.logging_steps = std::max(1, cli.logging_steps);
        ...
        trainer.save_lora(final_lora_path);
