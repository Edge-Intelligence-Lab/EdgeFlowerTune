...
enum class DiskQuantizationMode {
    None = 0,
    FP16 = 1,
    INT8 = 2,
    INT4 = 3,
...
struct ShardConfig {
    ...
    DiskQuantizationMode disk_quantization = DiskQuantizationMode::None;
};
...
struct ShardEntry {
    ...
    std::vector<uint8_t> encoded; // In-memory storage buffer when using memory-backed sharding
    DiskQuantizationMode disk_quantization = DiskQuantizationMode::None;
...
public:
    ...
    // Register a parameter that is already encoded in sharder on-disk/in-memory format.
    void register_encoded(const std::string& name,
                          const std::vector<int64_t>& shape,
                          DType dtype,
                          std::vector<uint8_t>&& encoded,
                          TensorPtr* owner_ptr = nullptr);
