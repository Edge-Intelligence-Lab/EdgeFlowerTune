    ...
    for (int64_t b = 0; b < B; ++b) {
        for (int64_t l = 0; l < L; ++l) {
            ...
            if (target_logit == -std::numeric_limits<float>::infinity()) {
                // The target class was not found in any chunk. Fall back to
                // log_sum_exp directly, which avoids producing NaNs here.
                // std::cerr << "[chunked_ce] target class not seen in chunks at ("<<b<<","<<l<<")"<<std::endl;
                total_loss += log_sum_exp; // Equivalent to an upper bound with target_logit=0.
            } else {
