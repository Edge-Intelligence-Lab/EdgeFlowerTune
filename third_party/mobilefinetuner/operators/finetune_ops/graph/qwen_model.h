...
public:
    ...
    QwenBlock& get_block(int idx) { return layers_.at(static_cast<size_t>(idx)); }
    const QwenBlock& get_block(int idx) const { return layers_.at(static_cast<size_t>(idx)); }
...
private:
    ...
    TensorPtr embedding_lookup(const TensorPtr& weight, const TensorPtr& indices) const;
