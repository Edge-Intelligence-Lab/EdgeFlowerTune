...
int main(int argc, char** argv) {
    ...
    if (argc >= 3 && argv[1] != nullptr && argv[2] != nullptr) {
        model_dir = argv[1];
        golden_path = argv[2];
        strict_mode = true;  // explicit args imply strict golden check
    } else {
        std::string env_model = path_utils::env_or_empty("GEMMA_270M_DIR");
        if (!env_model.empty()) model_dir = env_model;
        std::string env_golden = path_utils::env_or_empty("GEMMA_FORWARD_GOLDEN");
        if (!env_golden.empty()) golden_path = env_golden;
        ...
        if (model_dir.empty()) {
            ...
            for (const auto& dir : model_candidates) {
        ...
        if (golden_path.empty()) {
            const std::vector<std::string> golden_candidates = {
                "operators/finetune_ops/graph/testdata/gemma_forward_golden.bin",
                "finetune_ops/graph/testdata/gemma_forward_golden.bin",
                "../finetune_ops/graph/testdata/gemma_forward_golden.bin",
                "../../operators/finetune_ops/graph/testdata/gemma_forward_golden.bin"
            };
            for (const auto& p : golden_candidates) {
                if (fs::exists(p)) {
                    golden_path = p;
                    break;
                }
    ...
    if (model_dir.empty() || golden_path.empty()) {
        std::cerr << "Usage: " << argv[0] << " <model_dir> <golden_bin>" << std::endl;
        std::cerr << "Or set GEMMA_270M_DIR and GEMMA_FORWARD_GOLDEN." << std::endl;
        return 2;
    }

    const std::string strict_env = path_utils::env_or_empty("GEMMA_FORWARD_STRICT");
    if (strict_env == "1" || strict_env == "true" || strict_env == "TRUE") {
        strict_mode = true;
    }
    ...
    for (const auto& sample : samples) {
        auto encoded = tokenizer.encode(sample.text);
        if (encoded != sample.input_ids) {
    ...
    if (mismatch_count > 0) {
        std::cout << "[WARN] " << mismatch_count << " sample(s) exceed strict tolerance; "
                  << "treated as smoke pass (strict mode OFF)." << std::endl;
    } else {
        std::cout << "All samples matched." << std::endl;
    }
    std::cout << "Global max_err=" << global_max_err
