...
struct GemmaTextConfig {
    ...
    bool use_bf16_activations = false;  // Downcast activations to FP16 (enable when needed)
    int checkpoint_every = 0;  // Activation checkpointing: 0=off, N=every N blocks
    bool checkpoint_mlp = false;  // Checkpoint MLP subgraph to reduce activation memory
    int mlp_chunk_size = 0;  // MLP chunk size along seq (0=off)
...
class GemmaModel {
public:
    explicit GemmaModel(const GemmaTextConfig& config, bool allocate_base_tensors = true);
    ~GemmaModel() = default;

    TensorPtr forward(const TensorPtr& input_ids,
                      const TensorPtr& attention_mask = nullptr);
    TensorPtr forward_prefix(const TensorPtr& input_ids,
                             int split_layer = 0,
                             const TensorPtr& attention_mask = nullptr) const;
    TensorPtr lookup_token_embeddings(const TensorPtr& token_ids,
                                      bool scale_like_input = true) const;

    void assign_weight(const std::string& key, const TensorPtr& tensor);
    TensorPtr* weight_ref(const std::string& key);

    const GemmaTextConfig& config() const { return config_; }
    void set_auto_lm_head_tying(bool enabled) { auto_lm_head_tying_ = enabled; }
