...
void log_tensor_mem(const std::string& name, const TensorPtr& t) {
    if (!mem_debug_enabled() || !t) return;
    size_t bytes = static_cast<size_t>(t->numel()) * DTypeUtils::size_of(t->dtype());
    double mb = static_cast<double>(bytes) / (1024.0 * 1024.0);
    std::cout << "[MemDbg] " << name << " shape=" << shape_to_string(t->shape())
              << " dtype=" << DTypeUtils::to_string(t->dtype())
              << " bytes=" << bytes << " (" << std::fixed << std::setprecision(2)
              << mb << " MB)" << std::endl;
}

void ckpt_debug_tensor(const std::string& name, const TensorPtr& t) {
    if (!std::getenv("OPS_CKPT_DEBUG")) return;
    if (!t) {
        std::cout << "[CKPT] " << name << " = null" << std::endl;
        return;
    }
    std::cout << "[CKPT] " << name << " ptr=" << t.get()
              << " shape=" << shape_to_string(t->shape()) << std::endl;
}
...
TensorPtr ensure_fp32(const TensorPtr& t) {
    if (!t) return t;
    if (t->dtype() == kFloat32) return t;
    return cast(t, kFloat32);
}

TensorPtr maybe_fp16(bool enabled, const TensorPtr& t) {
    if (!enabled || !t) return t;
    if (t->dtype() == kFloat16) return t;
    return cast(t, kFloat16);
}
...
GPT2Model::GPT2Model(const GPT2Config& config)
    ...
    // In sharding-first mode, avoid eager full-weight allocation to reduce init RSS.
    // Parameters will be assigned by loader (and can be immediately offloaded).
    if (!config_.defer_parameter_allocation) {
...
TensorPtr* GPT2Model::parameter_ref(const std::string& key) {
    if (key == "wte.weight") return &wte_weight_;
    if (key == "wpe.weight") return &wpe_weight_;
    if (key == "ln_f.weight") return &ln_f_weight_;
    if (key == "ln_f.bias") return &ln_f_bias_;
    ...
    const int block_idx = std::stoi(match[1].str());
    const std::string sub_key = match[2].str();
    if (block_idx < 0 || block_idx >= config_.n_layer) {
        return nullptr;
    }
    auto& block = blocks_[block_idx];
    if (sub_key == "ln_1.weight") return &block.ln_1_weight;
    if (sub_key == "ln_1.bias") return &block.ln_1_bias;
    if (sub_key == "attn.qkv.weight") return &block.attn_qkv_weight;
    if (sub_key == "attn.qkv.bias") return &block.attn_qkv_bias;
    if (sub_key == "attn.proj.weight") return &block.attn_proj_weight;
    if (sub_key == "attn.proj.bias") return &block.attn_proj_bias;
    if (sub_key == "ln_2.weight") return &block.ln_2_weight;
    if (sub_key == "ln_2.bias") return &block.ln_2_bias;
    if (sub_key == "mlp.fc_in.weight") return &block.mlp_fc_in_weight;
    if (sub_key == "mlp.fc_in.bias") return &block.mlp_fc_in_bias;
    if (sub_key == "mlp.fc_out.weight") return &block.mlp_fc_out_weight;
    if (sub_key == "mlp.fc_out.bias") return &block.mlp_fc_out_bias;
    return nullptr;
}
...
TensorPtr GPT2Model::forward(const TensorPtr& input_ids,
    ...
    // FP16 activation storage is beneficial only when checkpointing is active.
    // Without checkpointing, extra cast nodes can increase retention.
    const bool fp16_activation_storage =
        config_.use_bf16_activations && (config_.checkpoint_every > 0 || config_.checkpoint_mlp);
    x = maybe_fp16(fp16_activation_storage, x);
    ...
    for (int i = 0; i < config_.n_layer; ++i) {
        if (config_.checkpoint_every > 0 && (i % config_.checkpoint_every) == 0) {
            int block_idx = i;
            auto ckpt_causal = causal;
            auto ckpt_pad = pad_mask;
            x = checkpoint([this, block_idx, ckpt_causal, ckpt_pad](const TensorPtr& inp) {
                return forward_block(inp, block_idx, ckpt_causal, ckpt_pad);
            }, x);
        } else {
    ...
    stage_rss("forward/ln_f");
    
    // 5. lm_head projection: logits = x @ wte^T, where wte is [V, C].
    // Use matmul_rhs_T to avoid materializing a transposed contiguous copy.
    if (sharder_) {
        sharder_->require("wte.weight");
        if (!wte_weight_) throw std::runtime_error("Sharder failed to keep wte for lm_head");
    }
    TensorPtr logits = matmul_rhs_T(x, wte_weight_);  // [B,S,C] @ [V,C]^T → [B,S,V]
    stage_rss("forward/end");
    if (rss_stage_enabled()) {
        rss_recorder.print("gpt2_forward");
    }
...
TensorPtr GPT2Model::forward_block(const TensorPtr& x_in,
    ...
    if (std::getenv("OPS_CKPT_DEBUG")) {
        std::cout << "[CKPT] forward_block start layer=" << block_idx
                  << " input=" << x_in.get() << std::endl;
    }
    ...
    TensorPtr x_fp32 = ensure_fp32(x_in);
    TensorPtr residual = x_fp32;
    TensorPtr x = layer_norm(x_fp32, w.ln_1_weight, w.ln_1_bias);
    ...
    stage_rss(stage_prefix + "attn_qkv");
    ...
    if (GPT2_DEBUG_PRINT && block_idx == 11) {
     ...
     // First reshape [B,S,3C] to [B,S,3,n_head,Hd], then slice/squeeze to avoid packed copies after slicing.
     auto qkv_5d = reshape(qkv, {B, S, 3, n_head, Hd});
     auto q = qkv_5d->slice(2, 0, 1)->squeeze(2);
     auto k = qkv_5d->slice(2, 1, 2)->squeeze(2);
     auto v = qkv_5d->slice(2, 2, 3)->squeeze(2);
    
    // DEBUG: Layer 11 QKV post-split statistics
    if (GPT2_DEBUG_PRINT && block_idx == 11) {
        auto calc_stats = [](const TensorPtr& t, const char* name) {
            TensorPtr t_work = t->is_contiguous() ? t : t->contiguous();
            const float* data = t_work->data<float>();
            double sum = 0.0, sum_sq = 0.0;
            for (int64_t i = 0; i < t_work->numel(); ++i) {
                sum += data[i];
                sum_sq += data[i] * data[i];
            }
            float mean = sum / t_work->numel();
            float std = std::sqrt(sum_sq / t_work->numel() - mean * mean);
    ...
    // Rearrange to multi-head: [B,S,n_head,Hd] -> [B,n_head,S,Hd]
    q = permute(q, {0, 2, 1, 3});  // view (no copy)
    k = permute(k, {0, 2, 1, 3});
    v = permute(v, {0, 2, 1, 3});
    stage_rss(stage_prefix + "attn_qkv_perm");
    // ========== Attention computation: standard attention only ==========
    TensorPtr context;
    const float scale = 1.0f / std::sqrt(static_cast<float>(Hd));

    auto scores = matmul_transB(q, k);   // [B,n_head,S,S] without explicit transpose
    scores = mul(scores, scale);
    log_tensor_mem("attn_scores", scores);
    stage_rss(stage_prefix + "attn_scores");
    ...
    // Fused mask + softmax to avoid extra intermediate SxS tensors.
    auto probs = masked_softmax(scores, causal_mask, pad_mask, -1);  // [B,n_head,S,S]
    scores.reset(); // release full attention score tensor ASAP
    log_tensor_mem("attn_probs", probs);
    stage_rss(stage_prefix + "attn_probs");

    // context = probs @ v
    context = matmul(probs, v);  // [B,n_head,S,Hd]
    log_tensor_mem("attn_context", context);
    stage_rss(stage_prefix + "attn_context");
    ...
    // Attention output projection: use LoRALinear
     ...
     stage_rss(stage_prefix + "attn_out");
     ...
     // Residual
    ckpt_debug_tensor("residual_attn", residual);
    ckpt_debug_tensor("attn_out", attn_out);
    ...
    stage_rss(stage_prefix + "attn_residual");
     ...
     auto mlp_forward = [this, block_idx, stage_prefix](const TensorPtr& input_mlp) -> TensorPtr {
         const auto& w = blocks_[block_idx];
         const int64_t seq_len = input_mlp->shape()[1];
         ...
         if (chunk <= 0 || chunk >= seq_len) {
             ...
             if (w.lora_initialized && w.fc_in_lin) {
                 h = w.fc_in_lin->forward(input_mlp);  // Includes LoRA increment
             } else {
                 h = matmul(input_mlp, w.mlp_fc_in_weight);
                 h = add(h, w.mlp_fc_in_bias);
             }
             stage_rss(stage_prefix + "mlp_fc_in");
             ...
             if (w.lora_initialized && w.fc_out_lin) {
                 out = w.fc_out_lin->forward(h);  // Includes LoRA increment
             } else {
                 out = matmul(h, w.mlp_fc_out_weight);
                 out = add(out, w.mlp_fc_out_bias);
             }
             stage_rss(stage_prefix + "mlp_fc_out");
             return out;
         }
         ...
         for (int64_t s0 = 0; s0 < seq_len; s0 += chunk) {
             ...
             auto x_chunk = input_mlp->slice(1, s0, s1);

             TensorPtr h;
             if (w.lora_initialized && w.fc_in_lin) {
                 h = w.fc_in_lin->forward(x_chunk);
             } else {
                 h = matmul(x_chunk, w.mlp_fc_in_weight);
                 h = add(h, w.mlp_fc_in_bias);
             }
             stage_rss(stage_prefix + "mlp_fc_in");
             h = gelu_new(h);
             ...
             TensorPtr out;
             if (w.lora_initialized && w.fc_out_lin) {
                 out = w.fc_out_lin->forward(h);
             } else {
                 out = matmul(h, w.mlp_fc_out_weight);
                 out = add(out, w.mlp_fc_out_bias);
             }
             stage_rss(stage_prefix + "mlp_fc_out");
             out_chunks.push_back(out);
         }
     ...
     if (config_.checkpoint_mlp) {
         mlp_out = checkpoint([mlp_forward](const TensorPtr& inp) { return mlp_forward(inp); }, x);
     } else {
         mlp_out = mlp_forward(x);
     }
     
     // Residual
    ckpt_debug_tensor("residual_mlp", residual);
    ckpt_debug_tensor("mlp_out", mlp_out);
    auto x_out = add(residual, mlp_out);
    stage_rss(stage_prefix + "mlp_residual");
    const bool fp16_activation_storage =
        config_.use_bf16_activations && (config_.checkpoint_every > 0 || config_.checkpoint_mlp);
    x_out = maybe_fp16(fp16_activation_storage, x_out);
    if (std::getenv("OPS_CKPT_DEBUG")) {
        std::cout << "[CKPT] forward_block end layer=" << block_idx
                  << " output=" << x_out.get() << std::endl;
    }
    return x_out;
 }
 ...
 void GPT2Model::print_model_info() const {
     ...
     std::cout << "  💾 use_fp16_activations: " << (config_.use_bf16_activations ? "ON" : "OFF") << std::endl;
     std::cout << "  🧠 checkpoint_every: " << config_.checkpoint_every << std::endl;
     std::cout << "  🧩 mlp_chunk_size: " << config_.mlp_chunk_size << std::endl;
 }
