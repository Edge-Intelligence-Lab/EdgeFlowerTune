...
int main() {
    ...
    // Numeric gradient check: use eps sweep to avoid finite-diff instability near softmax.
    auto eval_fd = [&](float eps) -> std::pair<float, float> {
        ...
        for (int64_t i = 0; i < k_in->numel(); ++i) {
            ...
            gk_num->data<float>()[i] = (lpos - lneg) / (2.0f * eps);
        }
        ...
        for (int64_t i = 0; i < v_in->numel(); ++i) {
            ...
            float lpos = lp->data<float>()[0];
            float lneg = ln->data<float>()[0];
            gv_num->data<float>()[i] = (lpos - lneg) / (2.0f * eps);
        }
    ...
    const std::vector<float> eps_list = {1e-2f, 3e-3f, 1e-3f, 3e-4f, 1e-4f, 3e-5f, 1e-5f};
    float best_eps = eps_list.front();
    float best_rdk = std::numeric_limits<float>::infinity();
    float best_rdv = std::numeric_limits<float>::infinity();
    float best_score = std::numeric_limits<float>::infinity();

    for (float eps : eps_list) {
        auto [rdk, rdv] = eval_fd(eps);
        float score = std::max(rdk, rdv);
        std::cout << "[RepeatKV+Softmax][eps=" << eps << "] rel_diff dK_in=" << rdk
                  << " dV_in=" << rdv << std::endl;
        if (score < best_score) {
            best_score = score;
            best_eps = eps;
    ...
    std::cout << "[RepeatKV+Softmax] best_eps=" << best_eps
              << " best_rel_diff dK_in=" << best_rdk
              << " dV_in=" << best_rdv << std::endl;

    const bool analytic_ok = (rdc < 1e-5f) && (rdd2 < 1e-6f) && (rdk_full2 < 1e-6f);
    const bool fd_ok = (best_rdk < 5e-2f) && (best_rdv < 5e-2f);
    bool ok = analytic_ok && fd_ok;
