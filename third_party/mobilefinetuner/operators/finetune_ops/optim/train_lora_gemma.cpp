...
int main(int argc, char** argv) {
    try {
        ...
        if (cli.pretokenized_path.empty()) {
            ...
            const std::string tokenizer_path = use_model_dir ? cli.model_dir : cli.tokenizer_json;
            tokenizer = std::make_unique<HFTokenizer>(tokenizer_path, use_model_dir);
            if (!tokenizer->load()) {
                throw std::runtime_error("Failed to load HF tokenizer from: " + tokenizer_path);
            }
        ...
        if (!cli.jsonl_train.empty() || !cli.jsonl_valid.empty() || !cli.jsonl_test.empty()) {
            // JSONL masked mode (highest priority)
            ...
            data_cfg.eos_id = tokenizer->token_to_id("<eos>");
            if (data_cfg.eos_id < 0) data_cfg.eos_id = tokenizer->token_to_id("</s>");
            if (data_cfg.eos_id < 0) {
                throw std::runtime_error("Failed to resolve Gemma EOS token from HF tokenizer");
            }
            data_cfg.pad_id = tokenizer->token_to_id("<pad>");
            if (data_cfg.pad_id < 0) data_cfg.pad_id = data_cfg.eos_id;
        } else {
