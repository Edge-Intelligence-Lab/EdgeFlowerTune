...
/** @brief Matrix multiplication with left-hand-side transpose: result = a^T @ b */
TensorPtr matmul_transA(const TensorPtr& a, const TensorPtr& b);
/** @brief Matrix multiplication with right-hand-side transpose: result = a @ b^T */
...
/** @brief Fused masked softmax for attention scores ([B,H,S,S], last-dim softmax). */
TensorPtr masked_softmax(const TensorPtr& scores,
                         const TensorPtr& causal_mask = nullptr,
                         const TensorPtr& pad_mask = nullptr,
                         int dim = -1);
...
TensorPtr apply_rope_contiguous(const TensorPtr& x, int seq_len, int head_dim, float rope_theta = 10000.0f);
