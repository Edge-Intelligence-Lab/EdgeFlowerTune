...
void accumulate_gradient(const TensorPtr& tensor, const TensorPtr& grad) {
    ...
    #ifdef USE_NEW_AUTOGRAD_ENGINE
    // Inject legacy grad_fn_ outputs into current engine state.
    autograd::Engine::instance().accumulate_external_grad(tensor, grad);
    return;
    #else
...
std::vector<TensorPtr> MatmulBackward::apply(const TensorPtr& grad_output) {
    ...
    if (a_ && a_->requires_grad()) {
        ...
        // Prefer transpose-aware kernels to avoid allocating contiguous transpose copies.
        if (b_shape.size() == go_shape.size()) {
            grad_a = matmul_transB(grad_output, b_);
        } else if (b_shape.size() == 2 && go_shape.size() >= 2) {
    ...
    if (b_ && b_->requires_grad()) {
        ...
        if (b_shape.size() == 2 && go_shape.size() >= 2) {
            ...
            int64_t n = go_shape[go_shape.size() - 1];
            int64_t batch = 1;
            ...
            if (weight_in_out) {
                ...
                grad_b = matmul_transA(a2d, go2d);
            } else if (weight_out_in) {
                // grad_W = grad_output^T @ X => [out, in]
                grad_b = matmul_transA(go2d, a2d);
            } else {
                grad_b = matmul_transA(a_, grad_output);
            }
        } else if (a_shape.size() == go_shape.size()) {
            grad_b = matmul_transA(a_, grad_output);
        } else {
...
std::vector<TensorPtr> RMSNormBackward::apply(const TensorPtr& grad_output) {
    ...
    TensorPtr input_read = input_->is_contiguous() ? input_ : input_->contiguous();
    TensorPtr grad_output_read = grad_output->is_contiguous() ? grad_output : grad_output->contiguous();
    const float* x = input_read->data<float>();
    const float* w = weight_->data<float>();
    const float* gy = grad_output_read->data<float>();
    float* gx = grad_input->data<float>();
    float* gw = grad_weight->data<float>();
...
std::vector<TensorPtr> SliceBackward::apply(const TensorPtr& grad_output) {
    ...
    const float* gout_f = (dtype == kFloat32) ? grad_output->data<float>() : nullptr;
    float* gin_f = (dtype == kFloat32) ? grad_input->data<float>() : nullptr;
    size_t elem_size = DTypeUtils::size_of(dtype);
    const char* gout_b = static_cast<const char*>(grad_output->data_ptr());
    char* gin_b = static_cast<char*>(grad_input->data_ptr());
    ...
    std::function<void(int,int64_t,int64_t)> scatter = [&](int d, int64_t out_offset, int64_t in_offset) {
        if (d == static_cast<int>(input_shape.size())) {
            if (dtype == kFloat32) {
                gin_f[in_offset] += gout_f[out_offset];
            } else {
                std::memcpy(gin_b + in_offset * elem_size,
                            gout_b + out_offset * elem_size,
                            elem_size);
            }
            return;
        }
...
std::vector<TensorPtr> RepeatKVHeadsBackward::apply(const TensorPtr& grad_output) {
    ...
    TensorPtr grad_read = grad_output->is_contiguous() ? grad_output : grad_output->contiguous();
    const float* src = grad_read->data<float>();
...
std::vector<TensorPtr> ApplyRoPEBackward::apply(const TensorPtr& grad_output) {
    ...
    TensorPtr grad_read = grad_output->is_contiguous() ? grad_output : grad_output->contiguous();
    const float* gy = grad_read->data<float>();
    float* gx = grad_input->data<float>();
...
std::vector<TensorPtr> ApplyRoPEContiguousBackward::apply(const TensorPtr& grad_output) {
    const auto& gshape = grad_output->shape();
    if (gshape.size() != 4) {
        throw TensorError("ApplyRoPEContiguousBackward expects 4D tensor [batch, heads, seq, head_dim]");
    }
    int64_t batch = gshape[0];
    int64_t heads = gshape[1];
    int64_t seq_len = gshape[2];
    int64_t head_dim = gshape[3];
    if (head_dim != head_dim_) {
        // Keep the same permissive behavior as ApplyRoPEBackward.
    }
    if ((head_dim % 2) != 0) {
        throw TensorError("ApplyRoPEContiguousBackward requires even head_dim");
    }
...
static TensorPtr slice_raw(const TensorPtr& input, int dim, int64_t start, int64_t end) {
    if (!input) return nullptr;
    const auto& shape = input->shape();
    int nd = static_cast<int>(shape.size());
    if (dim < 0) dim += nd;
    if (dim < 0 || dim >= nd) {
        throw TensorError("cat backward: slice dim out of range");
    }
    int64_t dim_size = shape[dim];
    if (start < 0) start += dim_size;
    if (end < 0) end += dim_size;
    start = std::max<int64_t>(0, std::min<int64_t>(start, dim_size));
    end = std::max<int64_t>(0, std::min<int64_t>(end, dim_size));
    int64_t slice_len = end - start;
    if (slice_len <= 0) {
        throw TensorError("cat backward: slice length must be positive");
    }

    auto new_shape = shape;
    new_shape[dim] = slice_len;
    auto result = std::make_shared<Tensor>(new_shape, input->dtype(), input->device());

    size_t elem_size = DTypeUtils::size_of(input->dtype());
    ...
    copy_slice = [&](int d, int64_t src_offset) {
        ...
        int64_t dim_end = (d == dim) ? end : shape[d];
...
std::vector<TensorPtr> CatBackward::apply(const TensorPtr& grad_output) {
    std::vector<TensorPtr> grads;
    if (!grad_output) {
        grads.resize(sizes_.size());
        return grads;
    }
    int64_t offset = 0;
    ...
    for (size_t i = 0; i < sizes_.size(); ++i) {
        ...
        auto g = slice_raw(grad_output, dim_, offset, offset + sz);
        grads.push_back(g);
        offset += sz;
    }
    return grads;
...
std::vector<TensorPtr> MatmulRhsTBackward::apply(const TensorPtr& grad_output) {
    // y = a @ b^T
    // grad_a = grad_y @ b
    // grad_b = grad_y^T @ a, summed over leading batch dims.
    std::vector<TensorPtr> grads;
    TensorPtr grad_a = nullptr;
    TensorPtr grad_b = nullptr;
    if (a_ && a_->requires_grad()) {
    ...
    if (b_ && b_->requires_grad()) {
        const auto& a_shape = a_->shape();
        const auto& go_shape = grad_output->shape();
        const auto& b_shape = b_->shape();  // expected [N, K]
        ...
        if (b_shape.size() == 2 && go_shape.size() >= 2) {
            ...
            auto a2d = reshape(a_, {batch * m, k});           // [B*M, K]
            auto go2d = reshape(grad_output, {batch * m, n}); // [B*M, N]
            grad_b = matmul_transA(go2d, a2d);                // [N, K]
        } else {
            grad_b = matmul_transA(grad_output, a_);
        }
...
std::vector<TensorPtr> MatmulTransABackward::apply(const TensorPtr& grad_output) {
    // y = a^T @ b
    // grad_a = b @ grad_y^T
    // grad_b = a @ grad_y
    TensorPtr grad_a = nullptr;
    TensorPtr grad_b = nullptr;
    if (a_ && a_->requires_grad()) {
        grad_a = matmul_transB(b_, grad_output);
    }
    if (b_ && b_->requires_grad()) {
        grad_b = matmul(a_, grad_output);
    }
    return {grad_a, grad_b};
}

std::vector<TensorPtr> MatmulTransBBackward::apply(const TensorPtr& grad_output) {
    // y = a @ b^T
    // grad_a = grad_y @ b
    // grad_b = grad_y^T @ a
    TensorPtr grad_a = nullptr;
...
std::vector<TensorPtr> CastBackward::apply(const TensorPtr& grad_output) {
    if (!grad_output) return {nullptr};
    auto grad_in = cast(grad_output, src_dtype_);
    return {grad_in};
}
