...
public:
    ...
    /**
     * @brief Accumulate gradient for a tensor from legacy grad_fn_ path.
     *
     * Used by operators that still execute custom grad_fn lambdas under the
     * new engine (e.g., memory_efficient_attention). The gradient is injected
     * into current backward state so upstream nodes can continue processing.
     */
    void accumulate_external_grad(const TensorPtr& tensor, const TensorPtr& grad);
    ...
    // When true, inputs that already belong to another graph (grad_node_ set)
    // will be treated as constants (no edge/registration). Used for checkpoint recompute.
    void set_suppress_external_grad_nodes(bool enabled) { suppress_external_grad_nodes_ = enabled; }
    bool suppress_external_grad_nodes() const { return suppress_external_grad_nodes_; }

    /**
     * @brief Save current engine state and clear internal maps.
     * This allows nested backward passes (e.g., activation checkpointing).
     */
    struct EngineState {
        std::unordered_map<const Tensor*, NodePtr> tensor_to_node;
        std::unordered_map<const Tensor*, TensorPtr> tensor_registry;
        std::unordered_map<const Tensor*, TensorPtr> pending_grads;
    };

    EngineState take_state();
...
private:
    ...
    bool enabled_;
    bool suppress_external_grad_nodes_ = false;
