...
public:
    ...
    int64_t numel() const;
    /** @brief Get logical strides (in elements) */
    const std::vector<int64_t>& strides() const { return strides_; }
    /** @brief Get stride for one dimension */
    int64_t stride(int dim) const { return strides_[static_cast<size_t>(dim)]; }
    /** @brief Get storage offset (in elements) from the root storage */
    int64_t storage_offset() const { return offset_; }
    /** @brief Check whether tensor layout is contiguous */
    ...
    /**
     * @brief Create a view tensor that shares storage with base tensor.
     * @param base Base tensor that owns underlying storage (or another view).
     * @param shape View shape.
     * @param strides View strides in elements.
     * @param offset Offset (in elements) relative to base logical data pointer.
     */
    static TensorPtr make_view(const TensorPtr& base,
...
private:
    ...
    TensorPtr view_base_ = nullptr;  // Keep storage alive for view tensors
    ...
    void copy_to_contiguous_buffer(void* dst) const;
