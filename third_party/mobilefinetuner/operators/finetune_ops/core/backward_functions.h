...
// Backward for y = a^T @ b
class MatmulTransABackward : public BackwardFunction {
private:
    TensorPtr a_;
    TensorPtr b_;
public:
    MatmulTransABackward(const TensorPtr& a, const TensorPtr& b) : a_(a), b_(b) {}
    std::vector<TensorPtr> apply(const TensorPtr& grad_output) override;
};

// Backward for y = a @ b^T (batched)
class MatmulTransBBackward : public BackwardFunction {
private:
    TensorPtr a_;
    TensorPtr b_;
public:
    MatmulTransBBackward(const TensorPtr& a, const TensorPtr& b) : a_(a), b_(b) {}
    std::vector<TensorPtr> apply(const TensorPtr& grad_output) override;
};

// Backward for cast
class CastBackward : public BackwardFunction {
private:
    DType src_dtype_;
public:
...
class ApplyRoPEContiguousBackward : public BackwardFunction {
private:
    int head_dim_;
    float rope_theta_;
public:
    ApplyRoPEContiguousBackward(int seq_len [[maybe_unused]], int head_dim, float rope_theta)
        : head_dim_(head_dim), rope_theta_(rope_theta) {}
    std::vector<TensorPtr> apply(const TensorPtr& grad_output) override;
};
...
class CatBackward : public BackwardFunction {
private:
    int dim_;
    std::vector<int64_t> sizes_;
public:
    CatBackward(int dim, const std::vector<int64_t>& sizes)
