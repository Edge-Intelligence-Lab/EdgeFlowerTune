...
namespace {
    ...
    int64_t batch_offset_for(const TensorPtr& t, int64_t batch_flat) {
        ...
        for (int d = ndim - 3; d >= 0; --d) {
            int64_t idx = batch_flat % shape[static_cast<size_t>(d)];
            batch_flat /= shape[static_cast<size_t>(d)];
            off += idx * strides[static_cast<size_t>(d)];
        }
        return off;
    }

    inline TensorPtr contiguous_if_needed(const TensorPtr& t) {
        if (!t) return t;
    ...
    TensorPtr elementwise_binary_op(const TensorPtr& a, const TensorPtr& b, Op op) {
        if (!a || !b) {
            throw TensorError("elementwise_binary_op: input tensor is null");
        }
        ...
        TensorPtr a_read = contiguous_if_needed(a);
        TensorPtr b_read = contiguous_if_needed(b);

        auto result_shape = broadcast_shapes(a, b);
        auto result = zeros(result_shape, a->dtype(), a->device());

        if (shapes_equal(a, b)) {
            const float* data_a = a_read->data<float>();
            const float* data_b = b_read->data<float>();
            ...
            auto shape_a = a_read->shape();
            auto shape_b = b_read->shape();
    ...
    TensorPtr elementwise_unary_op(const TensorPtr& x, Op op) {
        TensorPtr x_read = contiguous_if_needed(x);
        auto result = zeros(x->shape(), x->dtype(), x->device());

        const float* x_data = x_read->data<float>();
        float* result_data = result->data<float>();
...
TensorPtr matmul(const TensorPtr& a, const TensorPtr& b) {
    ...
    if (k != shape_b[shape_b.size() - 2]) {
        ...
        error_msg += "  A's K dimension=" + std::to_string(k);
    ...
    TensorPtr a_work = a;
    TensorPtr b_work = b;
    if (a_work->stride(static_cast<int>(a_work->ndim() - 1)) != 1) a_work = a_work->contiguous();
    if (b_work->stride(static_cast<int>(b_work->ndim() - 1)) != 1) b_work = b_work->contiguous();

    const float* data_a = a_work->data<float>();
    const float* data_b = b_work->data<float>();
    float* result_data = result->data<float>();

    const int64_t a_row_stride = a_work->stride(static_cast<int>(a_work->ndim() - 2));
    const int64_t a_col_stride = a_work->stride(static_cast<int>(a_work->ndim() - 1));
    const int64_t b_row_stride = b_work->stride(static_cast<int>(b_work->ndim() - 2));
    const int64_t b_col_stride = b_work->stride(static_cast<int>(b_work->ndim() - 1));

    auto naive_matmul_strided = [](const float* A, int64_t asr, int64_t asc,
                                   const float* B, int64_t bsr, int64_t bsc,
                                   float* C, int64_t m_, int64_t n_, int64_t k_) {
        for (int64_t i = 0; i < m_; ++i) {
            for (int64_t j = 0; j < n_; ++j) {
                float sum = 0.0f;
                for (int64_t p = 0; p < k_; ++p) {
                    sum += A[i * asr + p * asc] * B[p * bsr + j * bsc];
                }
                C[i * n_ + j] = sum;
            }
        }
    };
    auto rowmajor_matmul_accum = [](const float* A, int64_t asr,
                                    const float* B, int64_t bsr,
                                    float* C, int64_t m_, int64_t n_, int64_t k_) {
        // For very wide matrices such as Gemma lm_head [hidden, vocab], i-j-p
        // walks B with a huge stride. Accumulate C rows while scanning each B row
        // contiguously; C is already zero-initialized by zeros(...).
        for (int64_t i = 0; i < m_; ++i) {
            float* c_row = C + i * n_;
            const float* a_row = A + i * asr;
            for (int64_t p = 0; p < k_; ++p) {
                const float a_val = a_row[p];
                const float* b_row = B + p * bsr;
                for (int64_t j = 0; j < n_; ++j) {
    ...
    if (shape_a.size() == 2 && shape_b.size() == 2) {
        #ifdef USE_BLAS
        if (a_col_stride == 1 && b_col_stride == 1) {
            cblas_sgemm(CblasRowMajor,
                        CblasNoTrans, CblasNoTrans,
                        (int)m, (int)n, (int)k,
                        1.0f,
                        data_a, (int)a_row_stride,
                        data_b, (int)b_row_stride,
                        0.0f,
                        result_data, (int)n);
        } else {
            naive_matmul_strided(data_a, a_row_stride, a_col_stride,
                                 data_b, b_row_stride, b_col_stride,
                                 result_data, m, n, k);
        }
        #else
        if (a_col_stride == 1 && b_col_stride == 1) {
            rowmajor_matmul_accum(data_a, a_row_stride,
                                  data_b, b_row_stride,
                                  result_data, m, n, k);
        } else {
            naive_matmul_strided(data_a, a_row_stride, a_col_stride,
        ...
        const int64_t matrix_size_result = m * n;
        ...
        for (int64_t batch = 0; batch < total_batches; ++batch) {
            const int64_t off_a = batch_offset_for(a_work, batch);
            const int64_t off_b = b_has_batch ? batch_offset_for(b_work, batch) : 0;
            const float* batch_a = data_a + off_a;
            const float* batch_b = data_b + off_b;
            float* batch_c = result_data + batch * matrix_size_result;
            ...
            if (a_col_stride == 1 && b_col_stride == 1) {
                cblas_sgemm(CblasRowMajor,
                            ...
                            (int)m, (int)n, (int)k,
                            1.0f,
                            batch_a, (int)a_row_stride,
                            batch_b, (int)b_row_stride,
                            0.0f,
                            batch_c, (int)n);
            } else {
                naive_matmul_strided(batch_a, a_row_stride, a_col_stride,
                                     batch_b, b_row_stride, b_col_stride,
                                     batch_c, m, n, k);
            }
            #else
            if (a_col_stride == 1 && b_col_stride == 1) {
                rowmajor_matmul_accum(batch_a, a_row_stride,
                                      batch_b, b_row_stride,
                                      batch_c, m, n, k);
            } else {
                naive_matmul_strided(batch_a, a_row_stride, a_col_stride,
                                     batch_b, b_row_stride, b_col_stride,
...
TensorPtr matmul_transA(const TensorPtr& a, const TensorPtr& b) {
    if (!a || !b) {
        throw TensorError("matmul_transA: input tensors must not be null");
    }
    const auto& shape_a = a->shape();
    const auto& shape_b = b->shape();
    if (shape_a.size() < 2 || shape_b.size() < 2) {
        throw TensorError("matmul_transA: input tensors must be at least 2D");
    }
    if (shape_a.size() != shape_b.size()) {
        throw TensorError("matmul_transA: batch dims must match");
    }
    for (size_t i = 0; i + 2 < shape_a.size(); ++i) {
        if (shape_a[i] != shape_b[i]) {
            throw TensorError("matmul_transA: batch dims mismatch");
        }
    }

    int64_t m = shape_a[shape_a.size() - 2];
    int64_t k = shape_a[shape_a.size() - 1];
    int64_t m_b = shape_b[shape_b.size() - 2];
    int64_t n = shape_b[shape_b.size() - 1];
    if (m != m_b) {
        throw TensorError("matmul_transA dimension mismatch");
    }

    auto result_shape = shape_a;
    result_shape[result_shape.size() - 2] = k;
    result_shape[result_shape.size() - 1] = n;
    auto result = zeros(result_shape, a->dtype(), a->device());
    ...
    auto naive_matmul_transa = [](const float* A, int64_t asr, int64_t asc,
                                  const float* B, int64_t bsr, int64_t bsc,
                                  float* C, int64_t m_, int64_t n_, int64_t k_) {
        for (int64_t i = 0; i < k_; ++i) {
            for (int64_t j = 0; j < n_; ++j) {
                float sum = 0.0f;
                for (int64_t p = 0; p < m_; ++p) {
                    sum += A[p * asr + i * asc] * B[p * bsr + j * bsc];
                }
                C[i * n_ + j] = sum;
            }
        }
    ...
    for (int64_t batch = 0; batch < batch_size; ++batch) {
        const int64_t off_a = batch_offset_for(a_work, batch);
        const int64_t off_b = batch_offset_for(b_work, batch);
        const float* batch_a = data_a + off_a;
        const float* batch_b = data_b + off_b;
        float* batch_c = result_data + batch * (k * n);
        ...
        if (a_col_stride == 1 && b_col_stride == 1) {
            ...
            naive_matmul_transa(batch_a, a_row_stride, a_col_stride,
                                batch_b, b_row_stride, b_col_stride,
                                batch_c, m, n, k);
        }
        #else
    ...
    if (a->requires_grad() || b->requires_grad()) {
        result->set_requires_grad(true);
        auto backward_fn = std::make_shared<MatmulTransABackward>(a, b);
        register_backward(result, {a, b}, backward_fn);
    }
    #else
    if (a->requires_grad() || b->requires_grad()) {
        ...
        result->set_grad_fn([a, b](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
            ...
            if (a->requires_grad()) {
                grad_a = matmul_transB(b, grad_output);
                accumulate_gradient(a, grad_a);
            }
            if (b->requires_grad()) {
                grad_b = matmul(a, grad_output);
...
TensorPtr matmul_transB(const TensorPtr& a, const TensorPtr& b) {
    if (!a || !b) {
        throw TensorError("matmul_transB: input tensors must not be null");
    }
    const auto& shape_a = a->shape();
    const auto& shape_b = b->shape();
    if (shape_a.size() < 2 || shape_b.size() < 2) {
        throw TensorError("matmul_transB: input tensors must be at least 2D");
    }
    if (shape_a.size() != shape_b.size()) {
        throw TensorError("matmul_transB: batch dims must match");
    }
    for (size_t i = 0; i + 2 < shape_a.size(); ++i) {
        if (shape_a[i] != shape_b[i]) {
            throw TensorError("matmul_transB: batch dims mismatch");
        }
    }

    int64_t m = shape_a[shape_a.size() - 2];
    int64_t k = shape_a[shape_a.size() - 1];
    int64_t n = shape_b[shape_b.size() - 2];
    int64_t k_b = shape_b[shape_b.size() - 1];
    if (k != k_b) {
        throw TensorError("matmul_transB dimension mismatch");
    }
    ...
    auto naive_matmul_transb = [](const float* A, int64_t asr, int64_t asc,
                                  const float* B, int64_t bsr, int64_t bsc,
                                  float* C, int64_t m_, int64_t n_, int64_t k_) {
        for (int64_t i = 0; i < m_; ++i) {
            for (int64_t j = 0; j < n_; ++j) {
                float sum = 0.0f;
                for (int64_t p = 0; p < k_; ++p) {
                    sum += A[i * asr + p * asc] * B[j * bsr + p * bsc];
                }
                C[i * n_ + j] = sum;
            }
        }
    ...
    for (int64_t batch = 0; batch < batch_size; ++batch) {
        ...
        const float* batch_a = data_a + off_a;
        const float* batch_b = data_b + off_b;
        float* batch_c = result_data + batch * (m * n);
        ...
        if (a_col_stride == 1 && b_col_stride == 1) {
            ...
            naive_matmul_transb(batch_a, a_row_stride, a_col_stride,
                                batch_b, b_row_stride, b_col_stride,
                                batch_c, m, n, k);
        }
        #else
    ...
    if (a->requires_grad() || b->requires_grad()) {
        result->set_requires_grad(true);
        auto backward_fn = std::make_shared<MatmulTransBBackward>(a, b);
        register_backward(result, {a, b}, backward_fn);
    }
    #else
    if (a->requires_grad() || b->requires_grad()) {
        ...
        result->set_grad_fn([a, b](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
            ...
            TensorPtr grad_b = nullptr;
            if (a->requires_grad()) {
                grad_a = matmul(grad_output, b);
                accumulate_gradient(a, grad_a);
            }
            if (b->requires_grad()) {
                grad_b = matmul_transA(grad_output, a);
                accumulate_gradient(b, grad_b);
            }
            return {grad_a, grad_b};
        });
...
TensorPtr matmul_rhs_T(const TensorPtr& a, const TensorPtr& b) {
    ...
    if (shape_a.size() == 2) {
            ...
            for (int64_t i = 0; i < m; ++i) {
                for (int64_t j = 0; j < n; ++j) {
                    ...
                    for (int64_t p = 0; p < k_a; ++p) {
                        sum += data_a[i * a_row_stride + p * a_col_stride] *
                               data_b[j * b_row_stride + p * b_col_stride];
                    }
        ...
        for (int64_t batch = 0; batch < batch_size; ++batch) {
            const float* batch_a = data_a + batch_offset_for(a_work, batch);
            ...
            if (a_col_stride == 1 && b_col_stride == 1) {
                ...
                for (int64_t i = 0; i < m; ++i) {
                    for (int64_t j = 0; j < n; ++j) {
                        float sum = 0.0f;
                        for (int64_t p = 0; p < k_a; ++p) {
                            sum += batch_a[i * a_row_stride + p * a_col_stride] *
                                   data_b[j * b_row_stride + p * b_col_stride];
                        }
...
TensorPtr permute(const TensorPtr& tensor, const std::vector<int>& dims) {
    ...
    for (int i = 0; i < ndim; ++i) {
        int dd = dims[i] < 0 ? dims[i] + ndim : dims[i];
        new_strides[i] = src_strides[dd];
    }
    auto result = Tensor::make_view(tensor, new_shape, new_strides, 0);
...
TensorPtr cat(const std::vector<TensorPtr>& tensors, int dim) {
    if (tensors.empty()) {
        throw TensorError("cat: tensors list is empty");
    }
    auto ref = tensors.front();
    if (!ref) throw TensorError("cat: tensor is null");
    const auto& ref_shape = ref->shape();
    int ndim = static_cast<int>(ref_shape.size());
    if (ndim == 0) {
        throw TensorError("cat: cannot concatenate scalars");
    }
    if (dim < 0) dim += ndim;
    if (dim < 0 || dim >= ndim) {
        throw TensorError("cat: dim out of range");
    }

    // Validate shapes/dtype/device
    DType dtype = ref->dtype();
    Device device = ref->device();
    std::vector<int64_t> out_shape = ref_shape;
    int64_t out_dim = ref_shape[dim];
    std::vector<int64_t> sizes;
    sizes.reserve(tensors.size());
    sizes.push_back(ref_shape[dim]);
    for (size_t i = 1; i < tensors.size(); ++i) {
        if (!tensors[i]) throw TensorError("cat: tensor is null");
        if (tensors[i]->dtype() != dtype) throw TensorError("cat: dtype mismatch");
        if (tensors[i]->device() != device) throw TensorError("cat: device mismatch");
        const auto& shape = tensors[i]->shape();
        if (shape.size() != ref_shape.size()) {
            throw TensorError("cat: rank mismatch");
        }
        for (int d = 0; d < ndim; ++d) {
            if (d == dim) continue;
            if (shape[d] != ref_shape[d]) {
                throw TensorError("cat: shape mismatch");
            }
        }
        out_dim += shape[dim];
    ...
    auto result = std::make_shared<Tensor>(out_shape, dtype, device);

    // Compute outer/inner sizes for contiguous copy
    int64_t outer = 1;
    for (int d = 0; d < dim; ++d) outer *= ref_shape[d];
    int64_t inner = 1;
    for (int d = dim + 1; d < ndim; ++d) inner *= ref_shape[d];

    size_t elem_size = DTypeUtils::size_of(dtype);
    char* out_ptr = static_cast<char*>(result->data_ptr());

    for (int64_t o = 0; o < outer; ++o) {
        int64_t out_base = o * out_dim * inner;
        int64_t out_cursor = out_base;
        for (size_t ti = 0; ti < tensors.size(); ++ti) {
            const auto& t = tensors[ti];
            int64_t in_dim = t->shape()[dim];
            int64_t copy_elems = in_dim * inner;
            int64_t in_base = o * in_dim * inner;
            const char* in_ptr = static_cast<const char*>(t->data_ptr());
            std::memcpy(out_ptr + out_cursor * elem_size,
                        in_ptr + in_base * elem_size,
                        static_cast<size_t>(copy_elems) * elem_size);
            out_cursor += copy_elems;
        }
    }
    ...
    if (requires_grad) {
        result->set_requires_grad(true);
        auto backward_fn = std::make_shared<CatBackward>(dim, sizes);
        #ifdef USE_NEW_AUTOGRAD_ENGINE
        register_backward(result, tensors, backward_fn);
        #else
        result->set_grad_fn([backward_fn, tensors](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
            auto grads = backward_fn->apply(grad_output);
            for (size_t i = 0; i < tensors.size(); ++i) {
                if (tensors[i] && tensors[i]->requires_grad() && i < grads.size() && grads[i]) {
                    accumulate_gradient(tensors[i], grads[i]);
                }
            }
            return grads;
        });
...
TensorPtr masked_softmax(const TensorPtr& scores,
                         const TensorPtr& causal_mask,
                         const TensorPtr& pad_mask,
                         int dim) {
    if (dim != -1 && dim != scores->ndim() - 1) {
        throw TensorError("masked_softmax only supports last dimension currently");
    }
    if (scores->shape().size() != 4) {
        throw TensorError("masked_softmax expects scores shape [B,H,S,S]");
    }

    const auto& shape = scores->shape();
    const int64_t batch = shape[0];
    const int64_t n_head = shape[1];
    const int64_t seq_q = shape[2];
    const int64_t seq_k = shape[3];

    if (causal_mask) {
        const auto& cm_shape = causal_mask->shape();
        if (cm_shape.size() != 2 || cm_shape[0] != seq_q || cm_shape[1] != seq_k) {
            throw TensorError("masked_softmax: causal_mask must be [S,S] and match scores");
        }
    }

    int pad_mode = 0;  // 0=none, 2=[B,S], 4=[B,1,1,S]
    if (pad_mask) {
        const auto& pm_shape = pad_mask->shape();
        if (pm_shape.size() == 2 && pm_shape[0] == batch && pm_shape[1] == seq_k) {
            pad_mode = 2;
        } else if (pm_shape.size() == 4 && pm_shape[0] == batch && pm_shape[3] == seq_k) {
            pad_mode = 4;
        } else {
            throw TensorError("masked_softmax: pad_mask must be [B,S] or [B,1,1,S]");
        }
    }

    TensorPtr scores_read = contiguous_if_needed(scores);
    auto result = zeros(shape, scores->dtype(), scores->device());

    const float* s_data = scores_read->data<float>();
    float* out_data = result->data<float>();
    const float* cm_data = causal_mask ? causal_mask->data<float>() : nullptr;
    const float* pm_data = pad_mask ? pad_mask->data<float>() : nullptr;
    const auto& pm_strides = pad_mask ? pad_mask->strides() : std::vector<int64_t>{};

    for (int64_t b = 0; b < batch; ++b) {
        for (int64_t h = 0; h < n_head; ++h) {
            for (int64_t i = 0; i < seq_q; ++i) {
                const int64_t row_off = ((b * n_head + h) * seq_q + i) * seq_k;
                const float* row_in = s_data + row_off;
                float* row_out = out_data + row_off;

                float max_val = -std::numeric_limits<float>::infinity();
                for (int64_t j = 0; j < seq_k; ++j) {
                    float val = row_in[j];
                    if (cm_data) val += cm_data[i * seq_k + j];
                    if (pad_mode == 2) {
                        val += pm_data[b * seq_k + j];
                    } else if (pad_mode == 4) {
                        const int64_t pm_idx = b * pm_strides[0] + j * pm_strides[3];
                        val += pm_data[pm_idx];
                    }
                    row_out[j] = val;
                ...
                float sum = 0.0f;
                for (int64_t j = 0; j < seq_k; ++j) {
                    row_out[j] = std::exp(row_out[j] - max_val);
                    sum += row_out[j];
                }
    ...
    if (scores->requires_grad()) {
        ...
        auto backward_fn = std::make_shared<SoftmaxBackward>(result, dim);
        #ifdef USE_NEW_AUTOGRAD_ENGINE
        register_backward(result, {scores}, backward_fn);
        #else
        result->set_grad_fn([backward_fn, scores](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
            auto grads = backward_fn->apply(grad_output);
            if (scores->requires_grad()) {
                accumulate_gradient(scores, grads[0]);
            }
            return grads;
        });
        #endif
...
TensorPtr layer_norm(const TensorPtr& input, const TensorPtr& weight, const TensorPtr& bias, float eps) {
  ...
  TensorPtr rms_norm(const TensorPtr& input, const TensorPtr& weight, float eps) {
      ...
      auto result = zeros(input_shape, input->dtype(), input->device());
      TensorPtr input_read = contiguous_if_needed(input);
      const float* input_data = input_read->data<float>();
      const float* weight_data = weight->data<float>();
      float* result_data = result->data<float>();
  ...
  TensorPtr rms_norm_affine(const TensorPtr& input, const TensorPtr& weight, float eps) {
      ...
      auto result = zeros(input_shape, input->dtype(), input->device());
      TensorPtr input_read = contiguous_if_needed(input);
      const float* x = input_read->data<float>();
...
TensorPtr repeat_kv_heads(const TensorPtr& kv, int repeat_factor) {
    ...
    if (repeat_factor <= 0) {
        throw std::runtime_error("repeat_kv_heads expects repeat_factor > 0");
    }
    ...
    TensorPtr kv_read = contiguous_if_needed(kv);
    const float* kv_data = kv_read->data<float>();
    float* result_data = result->data<float>();
    ...
    const bool need_grad = kv->requires_grad();
    result->set_requires_grad(need_grad);
    #ifdef USE_NEW_AUTOGRAD_ENGINE
    if (need_grad) {
...
TensorPtr apply_rope_contiguous(const TensorPtr& x, int seq_len, int head_dim, float rope_theta) {
    // x shape: [batch, heads, seq_len, head_dim]
    // Qwen2/Qwen2.5 uses rotate_half-style RoPE: pair dim[d] with dim[d + head_dim/2].

    auto shape = x->shape();
    if (shape.size() != 4) {
        throw std::runtime_error("apply_rope_contiguous expects 4D tensor: [batch, heads, seq_len, head_dim]");
    }
    ...
    if ((head_dim % 2) != 0) {
        throw std::runtime_error("apply_rope_contiguous requires even head_dim");
    }
        ...
        for (int64_t h = 0; h < heads; ++h) {
            for (int64_t pos = 0; pos < seq_len; ++pos) {
                ...
                for (int64_t d = 0; d < half; ++d) {
                    float freq = 1.0f / std::pow(rope_theta, 2.0f * static_cast<float>(d) / static_cast<float>(head_dim));
                    float angle = static_cast<float>(pos) * freq;
                    float cos_val = std::cos(angle);
                    float sin_val = std::sin(angle);
                    int64_t idx1 = idx_base + d;
                    int64_t idx2 = idx_base + d + half;
                    float x1 = x_data[idx1];
                    float x2 = x_data[idx2];
                    result_data[idx1] = x1 * cos_val - x2 * sin_val;
                    result_data[idx2] = x2 * cos_val + x1 * sin_val;
                }
            }
        }
    }
    ...
    if (x->requires_grad()) {
        result->set_requires_grad(true);
        #ifdef USE_NEW_AUTOGRAD_ENGINE
        auto backward_fn = std::make_shared<ApplyRoPEContiguousBackward>(seq_len, head_dim, rope_theta);
        register_backward(result, {x}, backward_fn);
        #else
        result->set_grad_fn([x](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
            accumulate_gradient(x, grad_output);
...
TensorPtr cast(const TensorPtr& tensor, DType target_dtype) {
    ...
    if (src_dtype == kFloat32 && target_dtype == kFloat16) {
        ...
        for (int64_t i = 0; i < tensor->numel(); ++i) {
            ...
            if (v < -kFp16Max) v = -kFp16Max;
            dst[i] = float32_to_fp16(v);
        }
        if (tensor->requires_grad()) {
            result->set_requires_grad(true);
            auto backward_fn = std::make_shared<CastBackward>(src_dtype);
            #ifdef USE_NEW_AUTOGRAD_ENGINE
            register_backward(result, {tensor}, backward_fn);
            #else
            result->set_grad_fn([backward_fn, tensor](const TensorPtr& grad_output) -> std::vector<TensorPtr> {
                auto grads = backward_fn->apply(grad_output);
                if (tensor->requires_grad() && grads[0]) accumulate_gradient(tensor, grads[0]);
                return grads;
            });
            #endif
        }
