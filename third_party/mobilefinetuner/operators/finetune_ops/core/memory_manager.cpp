...
size_t read_env_mb(const char* key, size_t default_mb) {
...
MemoryPool::MemoryPool()
    : release_above_bytes_(read_env_mb("OPS_MEMPOOL_RELEASE_ABOVE_MB", 8)),
      size_buckets_(BUCKET_COUNT) {}
...
void MemoryPool::deallocate(void* ptr, size_t size [[maybe_unused]]) {
    ...
    for (auto it = blocks_.begin(); it != blocks_.end(); ++it) {
        ...
        if (block->ptr == ptr) {
            ...
            // For very large transient tensors, return memory to OS immediately
            // instead of caching in pool, which avoids long-lived RSS plateaus.
            if (!block->in_use &&
                release_above_bytes_ > 0 &&
                block->size >= release_above_bytes_) {
                size_t bucket_idx = get_bucket_index(block->size);
                auto& bucket = size_buckets_[bucket_idx];
                bucket.erase(std::remove(bucket.begin(), bucket.end(), block.get()), bucket.end());
