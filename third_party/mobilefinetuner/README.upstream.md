...
- Model binaries are built inside each model directory, for example:
  - `gpt2_small_lora_finetune/build/train`
  - `gpt2_medium_lora_finetune/build/train`
  - `gemma_3_270m_lora_finetune/build/train`
  - `gemma_3_1b_pt_lora_finetune/build/train`
  - `qwen_lora_finetune/build/train`
...
# Place in gpt2_small_lora_finetune/pretrained/gpt2/
```
...
- **Qwen2.5-0.5B:**
  ...
  qwen_lora_finetune/build/train \
    --data_dir data/wikitext2/wikitext-2-raw \
    --model_dir qwen_lora_finetune/pretrained \
    --seq_len 128 --batch 8 --grad_accum 1 \
    --steps 1000 --learning_rate 2e-4 \
    --lora_r 8 --lora_alpha 16 --lora_dropout 0.0
  ```

#### MMLU LoRA (masked JSONL)
Data prep: run the model-local `run_prepare_data.sh` script, for example `bash gpt2_small_lora_finetune/run_prepare_data.sh` or `bash qwen_lora_finetune/run_prepare_data.sh` (requires `transformers`), to generate MMLU JSONL files under `runs/mmlu_jsonl_*/{train,valid,test}.jsonl` with fields `ids` and `mask`.
...
- **GPT-2 Small / Medium:**
  ...
  ```
  Use `gpt2_medium_lora_finetune/build/train` with `gpt2_medium_lora_finetune/pretrained/gpt2-medium` for GPT-2 Medium.
- **Gemma 270M / 1B-PT:**
  ```bash
  gemma_3_1b_pt_lora_finetune/build/train \
    --model_dir gemma_3_1b_pt_lora_finetune/pretrained \
  ...
  ```
  Use `gemma_3_270m_lora_finetune/build/train` with the matching pretrained and MMLU JSONL directory for Gemma 270M.
- **Qwen2.5-0.5B:**
  ```bash
  qwen_lora_finetune/build/train \
    --jsonl_train runs/mmlu_jsonl_qwen_s128/train.jsonl \
    --jsonl_valid runs/mmlu_jsonl_qwen_s128/valid.jsonl \
    --model_dir qwen_lora_finetune/pretrained \
    --seq_len 128 --batch 8 --grad_accum 1 \
    --steps 1000 --learning_rate 2e-4 --target_mode qv
  ```
...
For numerical validation and debugging, MobileFineTuner includes PyTorch reference implementations in `PT_code/pytorch_alignment/`:

```bash
# GPT-2 LoRA (PyTorch baseline)
python PT_code/pytorch_alignment/gpt2_lora_finetune.py \
  --data_dir data/wikitext2/wikitext-2-raw \
  --pretrained_dir gpt2_small_lora_finetune/pretrained/gpt2 \
...
python PT_code/pytorch_alignment/gemma_lora_finetune.py \
...
│   ├── run_{train,eval,prepare_data}.sh
│   └── CMakeLists.txt
├── qwen_lora_finetune/                 # Qwen LoRA (WikiText-2, MMLU)
│   ├── src/main.cpp
│   ├── run_train.sh
│   └── CMakeLists.txt
├── PT_code/
