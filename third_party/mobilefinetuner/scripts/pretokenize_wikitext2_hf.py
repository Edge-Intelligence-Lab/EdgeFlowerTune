#!/usr/bin/env python3
"""
Offline tokenize WikiText-2 with a Hugging Face tokenizer into a single int32 stream.
Writes <output_dir>/<output_name> and <output_dir>/meta.json.
"""
...
def pack_tokens(lines: List[str], tokenizer, insert_eos_between_lines: bool) -> List[int]:
    ids: List[int] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pretokenize WikiText-2 with a HF tokenizer.")
    parser.add_argument("--model_dir", required=True, help="Path to model/tokenizer files.")
    parser.add_argument("--data_dir", default="data/wikitext2/wikitext-2-raw", help="WikiText-2 raw folder.")
    parser.add_argument("--output_dir", default="data/wikitext2/pretokenized_hf", help="Where to write bin/meta.")
    parser.add_argument("--output_name", default="wt2_tokens.bin", help="Token stream filename.")
    parser.add_argument("--preview", type=int, default=50, help="Print first N tokens for sanity check.")
    parser.add_argument("--use_fast", action="store_true", help="Force the fast tokenizer backend.")
    parser.add_argument(
        "--no_insert_eos_between_lines",
        dest="insert_eos_between_lines",
        action="store_false",
        help="Do not add EOS between lines.",
    )
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=args.use_fast)
