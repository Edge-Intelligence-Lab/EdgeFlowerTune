...
class PackedWikiText(Dataset):
    ...
    def _pack(tokens: List[int], seq_len: int, stride: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for start in range(0, max(0, total + 1), stride):
            ...
            # HuggingFace causal LM shifts internally (logits[:-1] vs labels[1:]).
            # Keep labels identical to inputs to avoid double-shifting.
...
def build_linear_warmup_scheduler(
    ...
    *,
    ...
    warmup_steps = max(0, min(num_warmup_steps, total_steps))
